...
{
    public class AccountController : WeChatControllerBase
    {
        ...
        private Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                ...
                case AbpLoginResultType.InvalidPassword:
                    return new UserFriendlyException("登录失败", "用户名密码错误!");
                ...
                case AbpLoginResultType.UserEmailIsNotConfirmed:
                    return new UserFriendlyException("登录失败", "UserEmailIsNotConfirmedAndCanNotLogin");
                case AbpLoginResultType.LockedOut:
                    return new UserFriendlyException("登录失败", L("UserLockedOutMessage"));
                default: //Can not fall to default actually. But other result types can be added in the future and we may forget to handle it
                    Logger.Warn("未捕获登录失败原因: " + result);
